...
public class BrowserUtils {
     ...
     *
     * @param url
     *            the url where to perform the POST.
     * @param response
     *            the {@link HttpServletResponse}.
     * @param values
     *            the values to include in the POST.
     * @throws IOException
    ...
    public static void postUsingBrowser(String url, HttpServletResponse response, Map<String, String> values) throws IOException {
        ...
        writer.write("<html><head></head><body><form id='TheForm' action='" + StringEscapeUtils.escapeHtml4(url) + "' method='POST'>");

        for (String key : values.keySet()) {
            String encodedKey = StringEscapeUtils.escapeHtml4(key);
            String encodedValue = StringEscapeUtils.escapeHtml4(values.get(key));
            writer.write("<input type='hidden' id='" + encodedKey + "' name='" + encodedKey + "' value='" + encodedValue + "'/>");
        }
